...
test.describe.parallel('Form-authentication page tests - Verify page logic', async () => {
    test.beforeEach(async ({ mainPage, baseURL }) => {
        await mainPage.navigateUrl(new PageDataConstants(baseURL as string).formAuthenticationPage.getUrlPath());
        await mainPage.waitForPageLoad();
    });

    test('Perform authentication with provided credentials on page', async ({ formAuthenticationPage }) => {
        //Arrange
        const [username, password] = await formAuthenticationPage.getUsernameAndPassword();

        //Act
        await formAuthenticationPage.performAuthentication(username, password);
